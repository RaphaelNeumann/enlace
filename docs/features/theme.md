...
```ts
{
  ...
  fontPair: "boheme-cormorant",
}
```

The `boheme-cormorant` font pair (registered in `src/lib/theme/fonts.ts`) loads:
- **Display / script**: `Boheme Floral` (commercial font, licensed by the project owner) → self-hosted from `src/fonts/BohemeFloral.ttf` via `next/font/local`, exposed as `--font-display`. Floral swash caps and flowing decorative strokes — the most "wedding-invitation" feel available on the project.
...
No third script font is needed — `Boheme Floral` covers all calligraphic uses (couple names, section headings). The TTF file is committed under `src/fonts/`; replace it with another licensed face (and update the `localFont` import) to swap the look.
